# Long-Running Agent Design

Companion artifacts for the *Long-Running Agents* article series on LinkedIn.

## The Series

A four-part series on designing long-running agents, exploring human nature as a design heuristic. The series covers four structural dimensions: role and purpose, security as policy, skills and growth, and designed well-being.

| Part | Title | Artifact |
|------|-------|----------|
| Zero | [The Anatomy of a Long-Running Agent](https://www.linkedin.com/pulse/anatomy-long-running-agent-michael-butler-fg1se) | — |
| One  | [The Reconciliation Tax](https://www.linkedin.com/pulse/reconciliation-tax-what-happens-when-agents-nature-fights-butler-kstge) | [Stack Alignment Diagnostic](stack-alignment-diagnostic.md) |
| Two  | [Reasoning About Boundaries: The Policy Skill Pattern](https://www.linkedin.com/pulse/reasoning-boundaries-policy-skill-pattern-michael-butler-dfiwe) | [Policy Skill Pattern](part-two-policy-skill-pattern/) |
| Three | Skills and Growth | Coming soon |
| Four | Designed Well-being | Coming soon |

## What's in this Repo

Each article in the series includes a practical artifact: a diagnostic, template, pattern, or reference you can take into your next build. This repo hosts those artifacts.

## Author

[Michael Butler](https://www.linkedin.com/in/michaelbutler29) — Principal Deep Learning Architect, AWS Generative AI Innovation Center.

## License

This work is licensed under [CC BY 4.0](LICENSE.md).
