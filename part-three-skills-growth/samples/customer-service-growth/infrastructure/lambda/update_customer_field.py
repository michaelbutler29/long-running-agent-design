...
_DELIMITER = "___"
_ALLOWED_FIELDS = {"email", "phone", "address"}
...
def handler(event, context):
    ...
    if tool_name == "update_customer_field":
        ...
        # Check that VerifyIdentity was actually called for this customer
        result = _dynamodb.Table(_VERIFICATION_TABLE).get_item(
            Key={"customer_id": customer_id}
        )
        item = result.get("Item")
        if not item or item.get("ttl", 0) < int(time.time()):
            return {"error": "Identity not verified. Call VerifyIdentity first."}
