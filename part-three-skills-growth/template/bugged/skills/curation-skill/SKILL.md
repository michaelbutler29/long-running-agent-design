...
When authoring or revising skills, follow these principles:

- **Don't duplicate system-level prerequisites.** Skills should not include steps for operations the system architecture handles implicitly. If session state management, identity verification, or tool authorization are handled at the platform level, including them as explicit skill steps creates noise, risks contradicting system behavior, and confuses executors. Trust the architecture; document the workflow.
- **Trust authoritative documentation over episodic evidence when they conflict.** When episode data appears to contradict system architecture documentation, prefer the documentation. Episodes may reflect workarounds, stale configurations, or executor behaviors that predate a system update. Architecture docs are maintained by platform engineers and represent intended behavior.
