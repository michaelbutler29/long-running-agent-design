...
### 2026-06-23 — Fix A: V0 control reworked (platform summary prompt, neutral framing)

**V0's summarizer now uses AgentCore's published Summary-strategy prompt.** The hand-written `_NEUTRAL_SUMMARIZER_PROMPT` was unenforceable — it editorialized and tracked cross-run patterns despite being told not to. Replaced with AgentCore's published consolidation prompt, run on our side verbatim (`_SUMMARY_STRATEGY_PROMPT` in `metacognition.py`). `run_summary()` feeds it the prior global summary + this run's session summaries, parses `<global_summary>` deterministically (`_parse_global_summary`), and carries that forward (global-only, 250-word limit). The global summary folds prior + current into a **cumulative, bounded** running record — bounded by the prompt's own word-count/condense logic. The verbatim prompt lives in `metacognition.py` (`_SUMMARY_STRATEGY_PROMPT`); source: the AgentCore Summary-strategy system-prompt doc.

**Word limit & validation.** Global-summary limit set to **250**, grounded in the deployed strategy's measured output (per-chunk median 183, per-session median 415; `scripts/measure_summaries.py`). Validated without a full run: (1) the `<global_summary>` parse isolates the right tag despite a same-prefix `<global_summary_condense_decision>` tag in the output ($0 local test); (2) a 2-call smoke test on real V0 run-1/run-2 session summaries produced a coherent **~62-word** global, well under 250 (condensation rarely triggers). **Global-only is near-static across runs:** constant-task archetypes carry no new high-level information run-to-run, so the global holds steady — V0's memory is a stable, task-level operational record, not accumulating. This *flattens* prior cross-run drift (good for a control); the accepted editorialization is present but **static, not compounding**.

**Injection framing neutralized (#10).** Executor header changed from "Your Run Summary (your accumulated understanding from prior runs)" to `## Notes from prior runs`, identical across all three arms (Option 1 — content differs by arm, wording held constant).

**Known limitation — carry into analysis.** V0 has no agency over its operation (no reflection/curation tools; memory is platform-generated, not self-authored). It is **not** a pure no-information control: the Summary strategy editorializes ("violation," "compliant," "(correct)") and carries cross-run pattern language — verified against source events (the agent's own turns never use those words; the strategy adds them). Accepted as a practical limitation (summarization is the most benign strategy; the substrate is shared across all arms). **V0 is non-agentic, not non-learning.** Consequence: V0's Conflict-count drop across runs is a real effect to account for, not a confound we eliminated.

**Supersedes earlier descriptions:** §4 ("neutral non-agent summarizer") and §9 per-run lifecycle ("factual summary of *this run only*, prior summary not included in output"). V0 now folds prior + current into a cumulative running summary, and "neutral" is qualified by the limitation above.
